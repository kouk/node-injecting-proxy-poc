...
me.prototype.make_target = function(proxydata) {
  var that=this,
      decoded_cookie, req_id = proxydata.request_id, cookies = proxydata.req.cookies;
  try {
  ...
  if (/^[A-Za-z0-9:.-]+$/.test(domain)) {
    ...
    proxydata.on('redirect', function(r, proxyres, options, proxydata) {
       ...
       var newcookie = _.clone(decoded_cookie),
           ...
           newreqid = proxydata.request_id.split("R")[0] + "R" + _.sample(_.range(10), 5).join("");
       ...
       proxydata.normalize_url(r);
       that.handle_href(r, proxydata.options, proxydata);
       proxyres.headers.location = r.format();
    });
  }
};
