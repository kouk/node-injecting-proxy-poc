...
PROXY_SUFFIX=${1:-livelocal}
PROXY_CN=${2:-proxy.$PROXY_SUFFIX}
export PROXY_SUFFIX PROXY_CN
...
if ! [ -f certificate.pem ] ; then
    ...
    openssl req -new -subj "/CN=$PROXY_CN" -newkey rsa:2048 -out req.pem -keyout key.pem -nodes -reqexts v3_req_livelocal -config openssl.cnf -nodes -batch
